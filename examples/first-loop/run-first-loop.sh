...
if [[ ! -d "$LOOPKIT_TARGET_REPO/.git" ]]; then
  ...
  git -C "$LOOPKIT_TARGET_REPO" add package.json src/slugify.js test/slugify.test.js
  git -C "$LOOPKIT_TARGET_REPO" commit -m "seed first-loop sandbox" >/dev/null
fi
